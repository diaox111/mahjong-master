...
const LEVELS = [
  { name: 'Level 1', widthNum: 8, heightNum: 10,
    blockTypeData: {1: 3, 2: 3},
    tiles: [
      [1, 16, 20],
      [1, 28, 20],
      [1, 40, 20],
  ...
  { name: 'Level 2', widthNum: 8, heightNum: 10,
    blockTypeData: {8: 2, 9: 2, 15: 3, 16: 3, 17: 3, 18: 3, 19: 2, 20: 3, 21: 3},
    tiles: [
      [1, 8, 16], [1, 16, 16], [1, 24, 16], [1, 32, 16], [1, 40, 16], [1, 48, 16],
      [1, 8, 24], [1, 8, 32], [1, 8, 40],
      [1, 48, 24], [1, 48, 32], [1, 48, 40],
      [1, 16, 40], [1, 24, 40], [1, 32, 40], [1, 40, 40],
      [1, 4, 56], [1, 52, 56],
      [2, 4, 20], [2, 12, 20], [2, 4, 28], [2, 4, 36], [2, 12, 36],
      [2, 20, 20], [2, 36, 20], [2, 44, 20], [2, 52, 20],
      [2, 52, 28], [2, 52, 36], [2, 44, 36], [2, 36, 36], [2, 20, 36],
      [2, 5, 56], [2, 51, 56],
      [3, 8, 24], [3, 8, 32], [3, 16, 24], [3, 16, 32],
      [3, 48, 24], [3, 48, 32], [3, 40, 24], [3, 40, 32],
      [3, 6, 56], [3, 50, 56],
      [4, 4, 28], [4, 12, 28], [4, 44, 28], [4, 52, 28],
      [4, 7, 56], [4, 49, 56],
      [5, 8, 56], [5, 48, 56],
      [6, 9, 56], [6, 47, 56],
      [7, 10, 56], [7, 46, 56],
...
function initLevel(levelIdx) {
  ...
  cfg.tiles.forEach(function(t) {
    if (t[1] > maxColPx) maxColPx = t[1];   // t[1] = col
    if (t[2] > maxRowPx) maxRowPx = t[2];   // t[2] = row
  });

  const cellPx = 4;  // yang-server coord unit: each unit = 4 px in original cocos game
  const layoutW = maxColPx + cellPx;
  const layoutH = maxRowPx + cellPx;
  const scale = Math.min(1, (W - 24) / layoutW, (H - 100) / layoutH);
  ...
  cfg.tiles.forEach(function(t) {
    ...
    const rowPx = t[2];
    state.nodes.push({
      id: t[0] + '-' + colPx + '-' + rowPx + '-' + rand(0, 99999),
      type: 0,
      zIndex: 10 + t[0],
      layer: t[0],
